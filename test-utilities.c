...
int ph_test(void) {
  ...
  //    meaningful bytes in appropriate places
  // If you want to test one of the other functions, they can be any bytes
  uint8_t input_data[] = {0x02, 0x13, 0x01, 0x00};
  ...
  //size_t expected_header_length = 4;
  //bool expected_is_compressed = false;
  ...
  // Note, you don't _need_ the CHECK() functions like we used in CS211, you
  //    can just return 1 then print that there was an error
  if (config->is_valid != expected_is_valid) {
    // Test failed! Return 1 to signify failure
    free(config);
    return 1;
  }
  ...
  /*
  if (config->header_len != expected_header_length) {
      // Test failed! Return 1 to signify failure
      free(config);
      return 1;
  }

  if (config->is_compressed != expected_is_compressed) {
        // Test failed! Return 1 to signify failure
        free(config);
        return 1;
  }
...
int main(void) {
  ...
  int result_ph = ph_test();
  if (result_ph != 0) {
      // Make sure to print the name of which test failed, so you can go find it and figure out why
      printf("ERROR: ph_test failed\n");
      return 1;
    }
